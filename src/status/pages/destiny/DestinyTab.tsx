              ...
              <div className={styles.partnerAppearance}>
                ...
                {(partner.外貌 !== undefined && partner.外貌 !== '') || editEnabled ? (
                  ...
                  <div className={styles.appearanceRow}>
                    <span className={styles.appearanceLabel}>外貌</span>
                    <span className={styles.appearanceValue}>无</span>
                  </div>
                )}
                {/* 身体状态：各部位单独列出（胸部、腹部、阴部、臀部、腿部等）；仅当 身体情况 为对象且含键时显示内容 */}
                <Collapse title="身体状态" className={styles.appearanceCollapse}>
                  <div className={styles.bodyStateContent}>
                    {(() => {
                      // 兼容旧数据：身体情况 可能为对象（正确）或字符串（旧格式，与外貌重复）
                      const bodyCond = partner.身体情况 as
                        | Record<string, string>
                        | string
                        | undefined;
                      if (
                        bodyCond &&
                        typeof bodyCond === 'object' &&
                        !Array.isArray(bodyCond) &&
                        Object.keys(bodyCond).length > 0
                      ) {
                        return _.map(bodyCond, (desc, partName) => (
                      ...
                      if (typeof bodyCond === 'string' && bodyCond.trim() !== '') {
                        return (
                          <span className={styles.hiddenEntryPlaceholder}>
                            （身体情况应为各部位键值；当前为旧格式，请由 AI 按变量更新规则填写
                            胸部/腹部/阴部/臀部/腿部）
                          </span>
                        );
                      }
                ...
                {/* 着装：同上，无内容且非编辑时也显示「无」 */}
                {(partner.着装 !== undefined && partner.着装 !== '') || editEnabled ? (
                  renderEditableRow(
                    '着装',
                    `命定系统.关系列表.${name}.着装`,
                    partner.着装,
                  ...
                  <div className={styles.appearanceRow}>
                    <span className={styles.appearanceLabel}>着装</span>
                    <span className={styles.appearanceValue}>无</span>
                  </div>
                )}
              </div>
